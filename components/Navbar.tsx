...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const isAuthenticated =
    propIsAuthenticated !== undefined
      ? propIsAuthenticated || !!session
      : !!dbService.getAuth() || !!session;
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex justify-between items-center h-16'>
          ...
          <div className='hidden md:flex items-center space-x-8'>
            <Link
              href='/'
              className={`text-sm font-medium transition-colors ${isActive('/') ? 'text-primary-600' : 'text-zinc-600 dark:text-zinc-400 hover:text-primary-600'}`}
            >
              Home
            </Link>
            <Link
              href='/items'
              className={`text-sm font-medium transition-colors ${isActive('/items') ? 'text-primary-600' : 'text-zinc-600 dark:text-zinc-400 hover:text-primary-600'}`}
            >
              Marketplace
            </Link>
            {isAuthenticated && (
              <Link
                href='/add-item'
                className={`text-sm font-medium transition-colors ${isActive('/add-item') ? 'text-primary-600' : 'text-zinc-600 dark:text-zinc-400 hover:text-primary-600'}`}
              >
                Sell Item
              </Link>
            )}
          ...
          {isAuthenticated ? (
            <>
              ...
              <button
                ...
                className='w-full text-left block px-3 py-2 rounded-md text-base font-medium text-red-600 dark:text-red-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'
              >
                Logout
              </button>
            </>
            ...
            <>
              ...
              <Link
                href='/register'
                onClick={() => setIsMenuOpen(false)}
                className='block px-3 py-2 rounded-md text-base font-medium text-primary-600 dark:text-primary-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'
              >
                Create Account
              </Link>
            </>
